...
<g:if test="\${!${propertyName}}">
    <elm:message tipo="notFound" contenido="No se encontró el ${className} solicitado" />
</g:if>
<g:else>
    ...
    <script type="text/javascript">
        var validator = \$("#frm${className}").validate({
            ...
            , rules          : {
                ...
                ${p.name}: {
                    remote: {
                        url: "\${createLink(controller: '${className}', action: 'validar_unique_${p.name}_ajax')}",
                        type: "post",
                        data: {
                            id: "\${${propertyName}?.id}"
                        }
